-- Bot turns: insert SQL fallback lines on the first eligible tick.
-- Prior 2s + random(55%) pacing until 6s often collided with the 15s turn
-- timeout and 2s client tick interval, so bots hit skip before any line.
